...
function loadSettings() {
    try {
        const saved = JSON.parse(localStorage.getItem('contestWidgetSettings') || '{}');
        // If no platforms are selected, default to these three
        if (!saved.prefPlatforms || saved.prefPlatforms.length === 0) {
            saved.prefPlatforms = ['codeforces', 'codechef', 'leetcode'];
        }
        return saved;
    } catch { 
        return { prefPlatforms: ['codeforces', 'codechef', 'leetcode'] }; 
    }
}
